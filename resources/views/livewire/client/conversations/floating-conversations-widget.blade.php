...
<script type="application/json" id="client-fcw-initial-data">{!! json_encode($initialList) !!}</script>

<div x-data="clientFloatingConversationsData()">
    <div wire:ignore>
        ...
        <div x-show="panelOpen" x-cloak style="position:fixed;bottom:88px;right:24px;z-index:70;width:320px;max-height:460px;background:#fff;border:1px solid #E7E5E4;border-radius:12px;box-shadow:0 16px 40px rgba(0,0,0,0.2);overflow:hidden;display:flex;flex-direction:column;">
            ...
            <div x-show="activeUuid" x-cloak>
                ...
                <div id="client-fcw-scroll" style="height:320px;overflow-y:scroll;padding:12px;"></div>
...
<script>
function clientFloatingConversationsData() {
    return {
        panelOpen: false,
        activeUuid: null,
        activeName: '',
        ...
        subscribeTo: function (uuid) {
            ...
            window.Echo.private('conversation.' + uuid).listen('.message.sent', function (e) {
                var isMine = e.sender_type === 'client' && e.sender_id === window.__currentUserId;
        ...
        openChat: function (uuid, name) {
            ...
            var self = this;
            var container = document.getElementById('client-fcw-scroll');
            if (container) container.innerHTML = '';
            var row = this.list.find(function (c) { return c.uuid === uuid; });
            if (row) { row.unread_count = 0; this.recalcTotalUnread(); }

            fetch('/client/conversations/' + uuid + '/quick-messages', { headers: { 'Accept': 'application/json' }, cache: 'no-store' })
        ...
        sendMini: function () {
            ...
            this.miniInput = '';
            this.appendToMini({ sender_type: 'client', sender_id: window.__currentUserId, sender_name: window.__currentUserName, body: text });

            var csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
            var formData = new FormData();
            formData.append('body', text);

            fetch('/client/conversations/' + uuid + '/quick-send', {
                method: 'POST',
        ...
        sendMiniVoiceNote: function () {
            ...
            this.appendToMini({
                sender_type: 'client', sender_id: window.__currentUserId, sender_name: window.__currentUserName,
                body: '',
        ...
        appendToMini: function (e) {
            var container = document.getElementById('client-fcw-scroll');
            if (!container) return;
            var isMe = e.sender_type === 'client' && e.sender_id === window.__currentUserId;
        ...
        init: function () {
            var dataEl = document.getElementById('client-fcw-initial-data');
            var initialList = [];
