...
- ~~**Broader staff roles/permissions review**~~ — **RESOLVED.** See the new "STAFF ROLES & PERMISSIONS SYSTEM" addendum below. Full self-service role/permission management now exists; this is no longer a gap.
- **Cross-tenant IDOR pattern** — **RESOLVED**, see same addendum. All 13 flagged occurrences fixed.
...
minishlink/web-push               ← installed for PWA Phase 3, VAPID-signed browser push notifications
```
...
# ADDENDUM — STAFF ROLES & PERMISSIONS SYSTEM, PLUS CROSS-TENANT IDOR REMEDIATION

---

## STRATEGIC CONTEXT

This session began as a full-app audit of staff role/permission gating — the prior addendum had flagged this as an open gap (only ONE hardcoded `hasRole('company_owner')` check existed anywhere, in `ConversationDetail`). The audit confirmed the seeded `PermissionSeeder` permission matrix was entirely dormant across every module (Events, Tasks, Vendors, Budget, Guests, RSVP, Runsheet, Forms, Contracts, Invoices, Assets, Billing, Domain Settings, Staff) — every authenticated staff member had identical functional access regardless of assigned role.

The scope expanded significantly mid-session: rather than a fixed 5-role hardcoded permission matrix, the user wants `company_owner` to be able to create arbitrary custom-named roles, toggle individual permissions per role via a real settings screen, AND grant/revoke specific permissions to individual staff members on top of their role's defaults. This became a genuine self-service RBAC system, not just enforcement wiring — built on Spatie Permission's existing (previously unused) infrastructure, which already supported per-tenant custom role names natively.

A separate, smaller thread — a cross-tenant IDOR pattern (bare `exists:table,column` validation rules bypassing the `BelongsToTenant` global scope) — was discovered during the audit and deliberately deferred to its own pass at the user's request, then completed at the end of this session.

---

## NEW CRITICAL RULES TO ADD

85. **`Spatie\Permission\Models\Role::create()` HAS A DUPLICATE-NAME CHECK THAT DOES NOT RELIABLY SCOPE BY TEAM/TENANT IN THIS APP'S PACKAGE VERSION** — calling it to create a tenant-scoped role (e.g. `tenant_id: 5`) throws `RoleAlreadyExists` if a role with the same name exists ANYWHERE (including the `tenant_id: null` template rows this app seeds), even though no real duplicate exists for that specific tenant. **`Role::firstOrCreate([...])` does not have this problem** — Eloquent's `firstOrCreate()` internally uses `newModelInstance()->save()` rather than calling the model's overridden static `create()`, bypassing the buggy check entirely. **Rule: always use `Role::firstOrCreate()`, never `Role::create()`, anywhere in this app that programmatically creates a Spatie role.**
86. **THIS APP'S SPATIE TEAMS CONFIG USES `tenant_id` AS THE PHYSICAL TEAM-FOREIGN-KEY COLUMN NAME IN `model_has_roles` — NOT `team_id`.** Any raw query referencing `model_has_roles.team_id` directly (bypassing Spatie's own relationship/scope methods) silently matches zero rows rather than erroring, since the column simply doesn't exist under that name. This bug existed in `StaffList.php`'s pre-existing role-filter dropdown (found, not yet independently fixed at time of finding) and was freshly introduced — then caught via the user testing a real "coordinator shows 0 staff" symptom — in `RolePermissions.php`'s staff-count query during this session. **Any future direct `model_has_roles` column reference must use `tenant_id`, never `team_id`.**
87. **LIVEWIRE ONLY RE-RUNS A COMPONENT'S `mount()` ON THE FIRST FULL PAGE LOAD — EVERY SUBSEQUENT ACTION REQUEST RE-INSTANTIATES THE COMPONENT AND JUMPS STRAIGHT TO THE TARGET METHOD, SKIPPING `mount()` ENTIRELY.** A non-public (`protected`/`private`) property set only inside `mount()` therefore silently reverts to its uninitialized default on every action after the first, throwing `must not be accessed before initialization` the first time an action method reads it — this hit `RolePermissions.php`'s `protected int $tenantId` property in this exact way. **Fix pattern used throughout this app: recompute the value fresh via a private helper method** (e.g. `private function tenantId(): int { return auth()->user()->tenant_id; }`) **rather than caching it in a non-public property at all** — this is the same reasoning that already made `StaffList.php`'s existing `render()` recompute `$tenantId` locally instead of trusting its own `protected` property.
88. **ALPINE'S `x-show` TOGGLES AN ELEMENT'S `display` CSS PROPERTY BETWEEN `none` AND WHATEVER VALUE IT WOULD OTHERWISE HAVE — BUT IF CENTERING STYLES (`display:flex; align-items:center; justify-content:center`) ARE BAKED INTO THE SAME INLINE `style` ATTRIBUTE ALPINE IS TOGGLING, THE RESTORED DISPLAY VALUE UNRELIABLY LOSES THE FLEX-CENTERING**, leaving the element pinned top-left instead of centered. **Fix pattern (used for both Role and Delete modals in `role-permissions.blade.php`): split into two nested divs** — an outer element `x-show` ONLY ever toggles visibility on (nothing else in its style), and an inner element that is PERMANENTLY flex-centered regardless of Alpine's state, so centering can never be clobbered by the visibility toggle.
89. **`position:fixed` NORMALLY CENTERS RELATIVE TO THE BROWSER VIEWPORT, BUT IF ANY ANCESTOR ELEMENT HAS A CSS `transform` APPLIED (E.G. THIS APP'S SIDEBAR SLIDE-IN/OUT ANIMATION), BROWSERS CREATE A NEW CONTAINING BLOCK, AND EVERY `fixed` DESCENDANT THEN POSITIONS RELATIVE TO THAT ANCESTOR INSTEAD OF THE REAL VIEWPORT.** This only manifests once a modal is PERMANENTLY present in the DOM (as with Alpine-owned instant-open modals, built to fix the delay described in Rule 88's context) rather than conditionally inserted via `@if` (the old Livewire-driven pattern) — the old pattern happened to never nest inside the transformed sidebar ancestor at the moment it rendered, masking this bug until the instant-open refactor. **Fix: wrap any permanently-in-DOM modal in `<template x-teleport="body">...</template>`**, which physically relocates the modal's markup to be a direct child of `<body>` at runtime, guaranteeing it's never a descendant of a transformed ancestor.
90. **BLADE'S DIRECTIVE PARSER SCANS THE ENTIRE FILE TEXT FOR `@directive` PATTERNS, INCLUDING INSIDE `<script>` TAGS AND EVEN INSIDE JAVASCRIPT COMMENTS.** Writing the literal text `@error` inside a `//` JS comment (e.g. explaining "Livewire re-renders with the inline @error message") gets parsed as a real, unmatched Blade directive and produces a confusing "unexpected end of file, expecting @endif" compile error nowhere near the actual typo. **When a Blade syntax error's reported line doesn't correspond to any visible directive imbalance, check for the literal strings "@error", "@if", "@foreach" etc. appearing inside JS/CSS comments or string literals elsewhere in the file** before assuming the Blade template logic itself is broken.
91. **LARAVEL'S BARE `exists:table,column` VALIDATION RULE STRING QUERIES THE RAW TABLE DIRECTLY, BYPASSING ELOQUENT GLOBAL SCOPES — INCLUDING THIS APP'S `BelongsToTenant` SCOPE.** A rule like `'vendor_id' => 'exists:vendors,id'` accepts ANY vendor ID that exists ANYWHERE across ANY tenant, not just the current tenant's own vendors — a genuine cross-tenant IDOR, exploitable by anyone bypassing the UI's dropdown and submitting a raw request with another tenant's numeric ID directly. **Fixed across 13 occurrences in 8 files this session** (`CreateEvent`, `EventDetail`, `CreateTask`, `RunsheetManager`, `CreateContract`, `CreateInvoice`, `AssetDetail`, `VendorDetail`) using `Rule::exists('table', 'column')->where('tenant_id', $tenantId)` in place of the bare string rule. **This is now the required pattern for any future `exists:` validation rule against a tenant-scoped table in this app** — a bare string `exists:` rule against a `BelongsToTenant` table should be treated as a bug on sight.
92. **A QUEUED JOB RUNS IN A SEPARATE PHP PROCESS WITH NO REQUEST-BOUND TENANT CONTEXT.** `app()->bound('tenant.id')` is typically `false` inside a job, meaning `BelongsToTenant`'s global scope silently adds NO where-clause at all — this returns rows across EVERY tenant, not zero rows, which is easy to misjudge as a "fails safe" scenario when it's actually the opposite. Additionally, `app()->instance('tenant.id', ...)` is a plain bound instance, not scoped per-job, so a long-running queue worker process could in principle leak a stale tenant context from an earlier job into a later one. **Any Eloquent query inside a queued job that touches a `BelongsToTenant` model must explicitly call `->withoutGlobalScope('tenant')` and filter by an explicitly-passed tenant ID** — this pattern is used throughout `SendWebPushNotificationJob`, `WebPushChannel`, and `NotificationPreference::channelsFor()`'s push-subscription check (see the PWA addendum below).

---

## ARCHITECTURE: PERMISSION SYSTEM

- **Roles are now genuinely tenant-editable**, not hardcoded. `roles` table gained an `is_system` boolean column — `true` only for `company_owner`, which can never be renamed, deleted, or stripped of permissions by anyone, including another `company_owner`.
- **4 starter role templates** (`coordinator`, `finance`, `operations`, `social_media_manager`) are copied into every NEW tenant at signup time via `TenantService::create()`, fully pre-populated with sensible default permissions — but fully owner-editable afterward (rename, re-permission, or delete, provided no staff are currently assigned).
- **Existing tenants** (created before this system existed) were brought up to date via a one-time idempotent backfill command, `koordli:backfill-staff-role-templates`. A second, additive-only command, `koordli:resync-staff-role-templates`, exists for safely granting newly-added permissions (e.g. when `contracts.manage` etc. were added to the catalog mid-session) to existing tenants' roles WITHOUT ever removing an owner's own customizations — this is the command to re-run any time the permission catalog itself grows in the future.
- **`App\Services\PermissionService`** is the single canonical place any "can this staff member do X" decision is made anywhere in the app. Resolution order: `company_owner` (is_system role) → always true, no exceptions; explicit per-user override row exists → use it; otherwise → fall back to the role's own default permissions via Spatie.
- **Per-user overrides** — a new `staff_permission_overrides` table (`tenant_id`, `user_id`, `permission_name`, `granted` boolean) lets `company_owner` grant OR revoke a specific permission for one specific staff member, on top of whatever their role normally allows. Exposed in the Staff Edit screen, gated on `staff.roles.manage` (so only whoever can manage roles/permissions sees this section at all), hidden entirely for `is_system` (owner) users.
- **Permission catalog was expanded significantly** beyond the original seeded matrix to cover modules that didn't exist when it was first written: `contracts.manage`/`contracts.view`, `invoices.manage`/`invoices.view`, `billing.manage`/`billing.view`, `domain-settings.manage`, `assets.manage`, `conversations.create`/`conversations.delete`/`conversations.manage_participants`, `vendors.assign` (splitting "assign vendor to event, a financial commitment" out from general vendor edit), `guests.view` vs `guests.view.basic` (splitting full-PII guest records from name/status-only visibility, for the `social_media_manager` role specifically).
- **Confirmed role defaults, per explicit user decisions this session:**
  - `coordinator`: full Contracts ownership (create/edit/send, including amounts — negotiates directly with clients), Staff read-only visibility, explicitly excluded from vendor-application approval (owner-only, since approval creates a real login-enabled account)
  - `finance`: full Invoices ownership, Contracts view-only (for reference, not editing), Events/Vendors view-only, explicitly excluded from Billing/plan changes (owner-only)
  - `operations`: full Assets ownership (day-of-event logistics), explicitly excluded from `vendors.assign` (no financial-commitment access) — vendor assignment/unassignment sits with coordinator/finance only
  - `social_media_manager`: read-only across the board, `guests.view.basic` only (names/counts, never email/phone)
  - **All `*.delete` permissions are `company_owner`-only, full stop**, regardless of what any individual role's other permissions suggest — deliberate, since this is a hard-delete, no-undo app and that risk should sit with the owner alone
- **`ConversationDetail`'s old hardcoded `hasRole('company_owner')` gate** (the one gate that existed before this session) was replaced with two granular permissions, `conversations.delete` and `conversations.manage_participants`, routed through `PermissionService` like everything else — `isTenantAdmin()` is kept as a backward-compatible alias (returns true only if both new permissions are held, matching prior behavior exactly for the default case).
...
### Enforcement (Phase 3)
Every previously-ungated Livewire component action across the whole app — Events, Tasks, Vendors (directory/detail/applications), Budget, Guests, RSVP, Runsheet, Forms, Contracts, Invoices, Assets, Billing, Domain Settings, Staff — now calls `PermissionService::userCan()` before proceeding, with a toast-error denial on failure (matching the app's existing toast pattern) and an `abort_unless(..., 403)` page-level gate in each component's `mount()`/`render()`.

### Role & Permission Management screen (Phase 4)
New route `/staff/roles` → `App\Livewire\Tenant\Staff\RolePermissions`, gated on a new `staff.roles.manage` permission (only `company_owner` has it by default). Plain, functional UI per explicit user request — no visual polish beyond matching existing `krd-` design tokens:
- Role list with staff-count and permission-count per role, `company_owner` visually locked (🔒, no edit/delete controls rendered)
- Permission checklist per role, grouped by module, native checkboxes styled with `accent-color:#7C3AED` (this app's established checkbox convention)
- Create / rename / delete role, with delete blocked (toast error, not silent failure) if any staff are currently assigned that role
- **Instant, zero-network-round-trip role-switching and permission-toggling**, achieved by embedding all roles' full data as a JSON payload on initial page load and letting Alpine own all read/display interaction client-side; Livewire calls are `#[Renderless]` background syncs only, never triggering a re-render
- Sidebar nav entry ("Roles & Permissions") added under the existing "Staff" section, itself conditionally rendered only for staff holding `staff.roles.manage`

### InviteStaff bug fix (Phase 0)
`InviteStaff.php`'s role dropdown previously validated against `manager`/`staff` — neither of which are real seeded roles (the actual 5 are `company_owner`/`coordinator`/`finance`/`operations`/`social_media_manager`) — meaning selecting either would silently create a brand-new, permission-less orphan role. Fixed to dynamically list the tenant's actual roles (now including any custom ones an owner has created via the new screen above), with a last-owner-protection guard preventing a tenant's sole remaining `company_owner` from being demoted.

### Branded 403 page
`resources/views/errors/403.blade.php` — Laravel automatically renders this for every `abort(403)` app-wide, including all the new Phase 3 permission gates. Matches the app's design system, includes a "Back to Dashboard" link appropriate to whichever guard (staff/client/vendor/platform) is currently authenticated, or a generic homepage link if none.
...
```
Migrations: add_is_system_to_roles_table, create_staff_permission_overrides_table
app/Services/PermissionService.php                          ← new, canonical permission-check service
app/Models/Tenant/StaffPermissionOverride.php                ← new
app/Console/Commands/BackfillStaffRoleTemplates.php          ← new, one-time backfill for existing tenants
app/Console/Commands/ResyncStaffRoleTemplates.php            ← new, additive-only, safe to re-run anytime
                                                                 the permission catalog grows
app/Services/TenantService.php                                ← now copies ALL 5 staff role templates (not
                                                                 just company_owner) into every new tenant
database/seeders/PermissionSeeder.php                          ← permission catalog significantly expanded
app/Livewire/Tenant/Staff/{InviteStaff,StaffList}.php          ← StaffList's pre-existing team_id/tenant_id
                                                                 role-filter bug also fixed this session
app/Livewire/Tenant/Staff/RolePermissions.php                  ← new, the role/permission management screen
resources/views/livewire/tenant/staff/role-permissions.blade.php  ← new
resources/views/errors/403.blade.php                            ← new, branded 403 page
components/layout/tenant-sidebar.blade.php                    ← new "Roles & Permissions" nav entry

app/Livewire/Tenant/{Billing,Contracts,Invoices,Events,Tasks,Vendors,Budget,Assets,Guests,Rsvp,Runsheet,Forms}/*
                                                                ← every action across every one of these
                                                                 modules now enforces PermissionService::userCan()
app/Livewire/Tenant/DomainSettings.php                          ← now enforces domain-settings.manage
app/Livewire/Tenant/Conversations/ConversationDetail.php        ← hardcoded hasRole() replaced with two
                                                                 granular permissions (see Rules section above)
```
...
- **`guest-list.blade.php`'s Guest PII split is only half-wired.** The `guests.view` vs `guests.view.basic` permission split exists and is enforced on write actions, and the Blade view now correctly hides email/phone columns for `guests.view.basic`-only users (fixed this session) — but this required editing an existing Blade view, a narrower scope exception explicitly approved by the user for this one specific fix, not a general "no UI changes" reversal.
- **No dedicated UI exists for per-user permission overrides beyond the Staff Edit screen's inline section.** Fully functional, but there's no separate "manage overrides" page — by design, matches the original scope request.
- **Resource-level scoping (e.g. "coordinator can only manage events they're personally assigned to via EventTeam") was explicitly declined this session** — the user confirmed pure role→permission scoping (matching how the original seed was shaped) is sufficient for now; resource-level scoping was acknowledged as "a good future idea but too big a change for this pass."

---

# ADDENDUM — PROGRESSIVE WEB APP (PWA) SUPPORT, PHASES 1–3 COMPLETE

---

## STRATEGIC CONTEXT

Built in three explicitly user-scoped phases so Koordli can be installed like a native app on phones/desktops, across all three portals (staff/client/vendor) sharing one manifest/icon per tenant. Deliberately deferred to a separate, later production concern: no install prompt exists on the plain public marketing site, since installability only makes sense once someone's inside an actual tenant dashboard (the manifest is tenant-scoped).

A key early architectural finding shaped the whole build: `ResolveTenantByDomain` middleware does NOT run uniformly across all three guards' authenticated routes (only on login pages for Client/Vendor, and not at all for staff, which relies purely on session-based `ResolveTenantFromUser`). Rather than trying to unify that pre-existing inconsistency, the PWA routes sidestep it entirely by resolving tenant from **whichever guard is currently authenticated** (checking `auth('web')`, `auth('client')`, `auth('vendor')` in turn) — since every user model across all three guards carries `tenant_id`, this works reliably regardless of domain-routing quirks.

---

## NEW CRITICAL RULES TO ADD

93. **THIS APP'S TENANT-RESOLUTION MIDDLEWARE (`ResolveTenantByDomain`) IS NOT UNIFORMLY APPLIED ACROSS ALL THREE GUARDS' AUTHENTICATED ROUTES** — it only runs on Client/Vendor LOGIN pages, and never on staff routes at all (staff relies purely on session-based `ResolveTenantFromUser`, independent of domain). Any future feature needing reliable tenant context on an arbitrary authenticated route (regardless of guard) should resolve tenant via the currently-authenticated user's own `tenant_id` column directly (checking `auth('web')`/`auth('client')`/`auth('vendor')` in turn), NOT by assuming domain-based middleware has run — this is the pattern established by `PwaController::resolveTenant()`.
94. **A CONTENT-HASH-BASED CACHE KEY (`md5_file()`) IS REQUIRED FOR ANY GENERATED-ASSET CACHE KEYED OFF A USER-UPLOADED FILE PATH THAT CAN BE OVERWRITTEN IN PLACE** — a path-based hash (`md5($path)`) only changes if the file's location changes, not its content, so a tenant re-uploading their logo to the same storage path (the normal case) would silently keep serving the OLD cached icon forever with a path-based key. This was caught and fixed BEFORE shipping, during design review of `PwaController::icon()`'s resize-cache logic, not discovered as a live bug.
95. **THE SERVICE WORKER SCRIPT ITSELF MUST NEVER BE AGGRESSIVELY HTTP-CACHED, OR CACHE-VERSIONING LOGIC INSIDE IT MAY NEVER ACTUALLY REACH THE BROWSER** — if `sw.js` is served as a static file (default web server behavior) rather than through a route that can set `Cache-Control: no-cache, must-revalidate` explicitly, browsers may skip re-checking it entirely on subsequent loads, meaning a version bump baked into the script never gets picked up. **Fix used in Phase 2: converted `sw.js` from a static `public/` file into a real Laravel route** (`PwaController::serviceWorker()`), specifically to set that header. **Critical follow-up action required whenever this pattern is adopted: the old static file at the same path must be physically deleted**, or most web servers will continue serving the static file directly and never invoke the route at all, silently nullifying the fix.
96. **CACHE-VERSIONING FOR A SERVICE WORKER SHOULD BE TIED TO THE ACTUAL BUILD OUTPUT (E.G. VITE'S MANIFEST FILE HASH), NEVER A MANUALLY-INCREMENTED VERSION NUMBER A DEVELOPER HAS TO REMEMBER TO BUMP.** A forgotten manual bump means stale cached assets persist indefinitely after a real deploy — this app has already been bitten by a related stale-asset caching bug once (see the Vite dev-server module cache issue in the Conversations addendum, Rule 83), so PWA cache versioning was deliberately built to compute its version automatically from `public/build/.vite/manifest.json`'s own content hash, with graceful fallbacks if that file isn't found.
97. **PHP'S OPENSSL EXTENSION ON WINDOWS FREQUENTLY CANNOT GENERATE EC (ELLIPTIC CURVE) KEYS — REQUIRED FOR WEB PUSH'S VAPID AUTHENTICATION — WITHOUT AN EXPLICIT `OPENSSL_CONF` ENVIRONMENT VARIABLE POINTING AT A VALID `openssl.cnf` FILE**, even when other OpenSSL operations work fine; `openssl_pkey_new(['curve_name' => 'prime256v1'])` silently returns `false` rather than throwing a clear error. On XAMPP specifically, a usable config exists at `C:\xampp\php\extras\openssl\openssl.cnf` (NOT the Apache bin folder, despite that being a more commonly suggested location for other XAMPP OpenSSL issues). Setting `OPENSSL_CONF` as a permanent Windows environment variable (via System Properties, not a shell-session-scoped `set`/`$env:` command) and fully restarting the terminal resolves it — this is purely a local Windows dev-environment quirk with no bearing on Linux production deployment, where this issue doesn't occur.

---

## ARCHITECTURE: PWA SYSTEM

**Phase 1 — Installable app:**
- `app/Http/Controllers/PwaController.php` — `manifest()` and `icon()` methods, both resolving tenant via the currently-authenticated guard (Rule 93)
- Manifest is dynamically generated per-request (not a static file), white-label-aware via the EXISTING `FeatureGateService::canAccess($tenant, 'white_label')` pattern already used for emails — shows tenant's own name/logo if white-labeled, Koordli's otherwise
- Icon generation uses PHP's built-in GD extension (no new composer dependency for this part) to resize a single 512×512 source into whatever sizes the manifest declares (192, 512, plus a 180×180 for iOS's separate `apple-touch-icon` link tag, since Safari doesn't read the manifest's `icons` array at all). Any non-square source (e.g. a future tenant-uploaded logo) is padded to a square transparent canvas before scaling, to avoid a stretched/cropped result
- Resized icons are cached to `storage/app/pwa-icons/`, keyed by content hash (Rule 94) plus requested size
- Service worker (`public/sw.js` in Phase 1, later converted to a route in Phase 2 — see Rule 95) is deliberately narrow-scoped: caches ONLY static assets matching JS/CSS/image/font patterns, with an explicit never-cache list for `/livewire/` and `/broadcasting/auth` paths — this project has already been burned once by stale-caching bugs (Rule 83), so the scoping here was written defensively from the start rather than broadened later
- White-label reinstall detection: compares current vs. previously-known white-label status in `localStorage` (keyed per tenant slug), shows a dismissible one-time banner prompting reinstall if a tenant transitions from non-white-labeled to white-labeled, since an already-installed home-screen icon typically won't auto-update (especially on iOS)

**Phase 2 — Offline fallback + cache versioning:**
- `resources/views/pwa/offline.blade.php` — deliberately self-contained (no `@vite`, no Livewire), precached by the service worker on install so it's available with zero network connection; branded per-tenant same as the manifest
- `resources/views/pwa/service-worker.blade.php` — the service worker itself, now rendered dynamically via `PwaController::serviceWorker()` rather than a static file, specifically to set `Cache-Control: no-cache` (Rule 95) and to inject an auto-computed version string (Rule 96) into `CACHE_NAME`
- Full-page navigation requests get network-first-with-offline-fallback; static assets keep the Phase 1 stale-while-revalidate strategy

**Phase 3 — Push notifications:**
- New `push_subscriptions` table (polymorphic `notifiable_type`/`notifiable_id`, matching the EXISTING `NotificationPreference` pattern exactly — no new schema convention invented), `endpoint_hash` (sha256) used for the unique constraint since raw push endpoint URLs are too long to index directly
- VAPID keys generated via `php artisan koordli:generate-vapid-keys` (new command, wraps `minishlink/web-push`'s key generator), stored in `.env`
- **Wired into the EXISTING `NotificationDispatchService`/`KoordliNotification` system as a genuine new channel, not a parallel system**: `KoordliNotification::via()` now maps a `'push'` channel string to a new `App\Notifications\Channels\WebPushChannel` class; `toPush()` added alongside the existing `toMail()`/`toDatabase()`/`toBroadcast()` methods
- Push has no per-category toggle in the existing Notification Preferences UI (browser push permission is device-level, not per-category) — instead, `NotificationPreference::channelsFor()` was extended to automatically include `'push'` alongside `'database'` wherever in-app notifications are already enabled, but ONLY if the notifiable actually has a real subscription on file; no subscription means push is silently a no-op regardless
- Actual sending happens via a queued job, `SendWebPushNotificationJob` (Rule 92's `withoutGlobalScope('tenant')` pattern applies here), dispatched once per active subscription by `WebPushChannel::send()`
- Dead/expired subscriptions (HTTP 404/410 from the push service) are automatically deleted by the job on send failure, preventing indefinite retry against a permanently-gone endpoint
- Subscribe flow: a dismissible banner (matching the white-label reinstall banner's visual pattern) prompts for browser push permission once per device, calling a new `PushSubscriptionController` (`/push/subscribe`, `/push/unsubscribe`) to persist the subscription
...
```
php artisan koordli:generate-vapid-keys              ← one-time, prints VAPID keys to paste into .env
php artisan koordli:backfill-staff-role-templates    ← one-time, for tenants existing before the roles system
php artisan koordli:resync-staff-role-templates       ← safe to re-run anytime the permission catalog grows
php artisan koordli:cleanup-pwa-icons                 ← scheduled weekly, deletes cached icon files older
                                                          than 30 days (harmless to run anytime — anything
                                                          still in use regenerates automatically on next request)
```
...
resources/views/pwa/{offline,service-worker}.blade.php
app/Console/Commands/{GenerateVapidKeys,CleanupPwaIcons}.php
app/Models/Tenant/PushSubscription.php
...
config/services.php  ← new 'vapid' config block (public_key, private_key, subject)
routes/web.php        ← /manifest.webmanifest, /pwa/icon/{size}.png, /offline, /sw.js,
                         /push/subscribe, /push/unsubscribe
routes/console.php    ← koordli:cleanup-pwa-icons scheduled ->weekly()
resources/views/layouts/{tenant,client,vendor}.blade.php
                       ← manifest/apple-touch-icon link tags, window.__tenantWhiteLabel,
                         window.__tenantSlug, window.__vapidPublicKey globals injected
resources/js/app.js   ← service worker registration, white-label reinstall banner,
                         push subscribe banner + subscription logic
```
...
- **`storage/app/pwa-icons/` accumulates orphaned files over time** whenever a tenant changes their logo (old content-hash-keyed file is never referenced again, but isn't automatically deleted either) — the new `koordli:cleanup-pwa-icons` weekly command handles this; harmless disk-space-only concern, not a functional bug.
- **Tenant logo upload has no dimension/aspect-ratio validation** at the point of upload (outside this session's scope — that upload form wasn't touched). The icon-generation pipeline defensively pads any non-square source, but a genuinely low-resolution uploaded logo will still look poor as a home-screen icon; worth adding upload-time validation to the branding settings screen eventually.
- **Local Windows dev environment has no cron** — any `Schedule::command(...)->weekly()` etc. (including the new PWA icon cleanup) requires either manually running the command once, or running `php artisan schedule:work` to simulate the scheduler continuously, purely for local testing. Production deployment already documents the required real cron entry in `KOORDLI_PRODUCTION_TRAEFIK_SETUP.md`.

---

# ADDENDUM — VENDOR SELECTION & TRANSPARENCY, CLIENT/VENDOR NOTIFICATIONS, QUICK ACCESS LINKS, AND SELF-SERVICE ACCOUNT PAGES

---

## STRATEGIC CONTEXT

This was the largest single session in the project's history, covering four major features plus a long tail of bug fixes discovered along the way. Built in this order:

1. **Vendor Selection & Transparency** — replacing the informal "Instagram → WhatsApp → Google Sheets" vendor-sourcing workflow real event planners described, with a structured, tenant-configurable in-app flow.
2. **Client Notifications** (6 categories) and **Vendor Notifications** (4 categories) — extending the existing staff-only notification system (bell/dropdown/preferences) to the two portals that never had it, via a full trigger-point audit (same methodology as the original Roles & Permissions audit) rather than guessing at scope.
3. **RSVP response-percentage milestones** — a smaller, explicitly-deferred item from the notifications audit, closed out separately.
4. **Quick Access Links** — permanent, no-login, password-equivalent personal links for staff and vendors to update tasks/runsheet items from the field, with full audit logging, tenant/self-service regeneration, per-person optional PIN, and plan-gated via the existing `FeatureGateService`.
5. **Clients & Settings pages, plus self-service profile/password pages for all three portals** — closing out two long-dead `href="#"` sidebar placeholders and a real self-service gap (no portal except Vendor had ANY profile page, and even that one had no password-change capability).

A recurring theme worth naming explicitly: **almost every UI built in this session initially used a `wire:click`-driven modal/dropdown/checkbox and had to be corrected to Alpine-instant** after the fact, because the user has a zero-tolerance standard for interaction delay. By the end of the session this became a build-time discipline rather than a reactive fix — new interactive elements (the Quick Access admin table's bulk-select, the reusable confirm-button component) were built Alpine-first from the start.

---

## NEW CRITICAL RULES TO ADD

98. **BLADE INTERPOLATING A PHP NAMESPACED CLASS STRING (E.G. `App\Models\Tenant\User`) DIRECTLY INTO A `wire:click`/`x-on:click` ATTRIBUTE VALUE CORRUPTS IT.** Livewire's JS parses the attribute value as a JS expression; JavaScript silently drops backslashes it doesn't recognize as valid escape sequences (`\M`, `\T`, `\U`), turning `App\Models\Tenant\User` into `AppModelsTenantUser` — a real class name that then fails to resolve. **Always wrap a PHP string being passed into a Livewire action call with `@js()`** (e.g. `wire:click="regenerate(@js($row['person_type']), ...)"`), which correctly JSON-encodes it so backslashes survive. Same root cause as embedding any string containing backslashes, quotes, or other JS-significant characters into a raw Blade-interpolated JS context — this bug will recur anywhere a fully-qualified class name (or file path, or anything backslash-containing) is passed this way without `@js()`.
99. **`whereHas()` MUST NOT BE USED ON A `MorphTo` RELATIONSHIP.** Laravel's own documentation states this explicitly, but it's an easy trap: `whereHas('subject', fn($q) => $q->whereNotIn('status', [...]))` on a polymorphic `subject()` relation (which can resolve to `Task`, `VendorContract`, `RunsheetItem`, `QuickAccessLink`, etc. depending on the row) causes Eloquent to attempt building the SAME constraint against every model class it has ever seen associated with that morph column — including ones that don't have the referenced column at all (e.g. `QuickAccessLink` has no `status` column), producing a "column not found" SQL error that looks unrelated to the actual bug. **Use `whereHasMorph('subject', [Task::class], fn($q) => ...)` instead**, explicitly restricting which class(es) the constraint applies to.
100. **ON WINDOWS, CLOSING A TERMINAL RUNNING A LONG-LIVED PHP PROCESS (e.g. `php artisan reverb:start`) DOES NOT RELIABLY KILL THE UNDERLYING PROCESS.** It can keep holding its bound port silently in the background as an orphaned `php.exe`, invisible in the terminal but still occupying the port — so a subsequent `reverb:start` prints "Starting server" successfully while never actually being reachable, because an old zombie process already owns the port. Symptom: `Test-NetConnection` fails, Windows Firewall has zero relevant rules (ruling out firewall as the cause), yet `netstat -ano | findstr :<port>` shows MULTIPLE processes LISTENING on the same port. **Diagnostic sequence when a local WebSocket/Reverb connection silently fails on Windows:** (1) `Test-NetConnection -ComputerName 127.0.0.1 -Port <port>` to confirm reachability, (2) if it fails, check Windows Firewall rules for the relevant app name — if none exist, that's not necessarily the cause (test by briefly toggling the firewall off for Private network, then back on), (3) `netstat -ano | findstr :<port>` to check for MULTIPLE processes on the same port — if found, `Stop-Process -Id <pid> -Force` on each, verify `netstat` returns empty, then restart the service cleanly.
101. **PIN-PROTECTED PUBLIC DATA MUST NOT BE EMBEDDED IN THE PAGE PAYLOAD BEFORE THE PIN IS VERIFIED.** `QuickAccessPage`'s `render()` deliberately builds and JSON-embeds the person's task/event/runsheet data ONLY after `$this->pinVerified` is true — if this data were embedded on initial page load regardless of PIN state, anyone with the raw link (without the PIN) could view page source and see task titles, event names, etc., defeating the entire purpose of the PIN as an additional protection layer beyond just possessing the link.
102. **A "REGENERATE PERMANENT CREDENTIAL" ACTION SHOULD DISTINGUISH SELF-INITIATED FROM ADMIN-INITIATED IN ITS NOTIFICATION TEXT.** `QuickAccessLink::regenerateFor()` takes an `$initiatedBySelf` boolean specifically so the resulting notification says "You regenerated..." vs. "Your organization regenerated..." — without this distinction, a person whose link was force-regenerated by their tenant (e.g. after reporting it compromised) would see an identical, ambiguous message to one they triggered themselves, which is a real clarity gap for a security-relevant event. Any future "credential regenerated/revoked by either the owner or an admin" notification in this app should carry the same distinction.
103. **A DEDICATED SELF-SERVICE PROFILE PAGE (NAME + PASSWORD CHANGE) DID NOT EXIST FOR ANY PORTAL EXCEPT VENDOR, AND EVEN VENDOR'S HAD NO PASSWORD-CHANGE CAPABILITY, UNTIL THIS SESSION.** Staff had no self-service profile page at all (only `InviteStaff.php`, which is staff managing OTHER staff, and `Onboarding.php`, which only sets the initial password once at account creation). Client had neither. This is now closed for all three (`Staff\Profile`, extended `Vendor\Profile`, new `Client\Profile`), each following an identical pattern: name/contact fields editable, email always read-only (changing a login email has bigger implications — verification, uniqueness — not solved here), and a password-change form requiring current-password confirmation plus the same uppercase/lowercase/digit complexity rule already used in `Onboarding.php`.
104. **VENDOR AND CLIENT ACCOUNTS ARE TENANT-SCOPED PER-RELATIONSHIP, NOT GLOBAL IDENTITIES — THIS IS A STRUCTURAL FACT OF THE APP'S DESIGN, NOT SOMETHING ANY SINGLE FEATURE (LIKE A PROFILE PAGE) CAUSES OR FIXES.** If the same vendor/client works with two different tenants on Koordli, they get two completely separate accounts with separate logins, since `VendorAccount`/`Client` both carry a single `tenant_id` and account-creation only checks for an existing email WITHIN that one tenant. Explicitly discussed and deliberately left as-is this session — a genuine cross-tenant shared identity (one login working across every tenant relationship) would be a substantial future re-architecture, not something to bolt on as a side effect of any feature built here.

---

## ARCHITECTURE: VENDOR SELECTION & TRANSPARENCY

- **Extends existing models, no new core tables for the base flow.** `vendor_event_assignments` gained `selection_source` (planner_selected/client_selected/planner_suggested/client_external), `client_approval_status`, `client_approved_at`, `is_client_visible`, `client_can_view_pricing`, `payment_responsibility`, `disclaimer_acknowledged_at`. `vendors` gained `source` (directory/marketplace_application/client_added) and `added_by_client_id`. `tenants` gained `client_vendor_involvement_level` (none/view_only/approve_selections/full_participation) and `vendor_disclaimer_text`.
- **One genuinely new table: `vendor_suggestions`** — the negotiation/proposal-inbox layer sitting BEFORE a vendor becomes a real, budgeted `VendorEventAssignment`. Polymorphic `suggested_by_type/id` (Tenant\User or Central\Client), free-text `category_label` for client-facing input plus optional structured `vendor_category_id` mapping set on approval, `status` (pending/approved/rejected/withdrawn), and `resulting_vendor_id`/`resulting_assignment_id` populated only on approval.
- **Approval and finalization are two deliberately separate steps.** Someone approving a suggestion only signals "let's move forward" — it does NOT touch budget or create a real vendor record. A second, staff-only "Finalize" step (regardless of who approved) is what actually creates the `Vendor`/`VendorEventAssignment`, since money terms need staff judgment either way.
- **`selection_source` on finalization is decided by TWO factors, not one:** who suggested it (client vs. staff) AND whether it links to an EXISTING vetted directory vendor or creates a BRAND NEW one. A brand-new vendor sourced from a CLIENT finalizes as `client_external` (the real liability/disclaimer scenario); everything else finalizes as `client_selected` or `planner_suggested`.
- **Per-event override of the tenant-wide involvement level**, via a nullable `client_vendor_involvement_override` on `events` (NULL = inherit tenant default) — resolved through one canonical method, `Event::effectiveClientVendorInvolvementLevel()`, which every consumer (client's Vendor page, the dashboard's conditional link, the tenant's Suggestions button) must call rather than reading either column directly.
- **Selection badges** ("Client's Choice," "Suggested by You, Approved," "Client's Own Vendor") render on the ordinary Event Detail vendor card and Vendor Directory — deliberately visible to staff browsing NORMALLY, not buried in a separate suggestions-only inbox, so the client-driven nature of a booking is obvious at a glance without extra clicks.
- **"Discuss" reuses Conversations' existing polymorphic participant model** rather than building inline commenting on a suggestion — deliberate reuse over new infrastructure.
- **Client Vendor page has THREE gated tiers**, not a single on/off: `view_only` shows Confirmed Vendors only (no Suggestions section at all — shown to render dead-end "nothing to click" UI otherwise, a real bug found and fixed during the gating cross-check); `approve_selections` adds the ability to approve/reject staff suggestions; `full_participation` additionally allows the client to submit their own suggestions.

## ARCHITECTURE: CLIENT NOTIFICATIONS (6 categories)

Built via the same audit-first methodology as Roles & Permissions — every category's trigger point was individually confirmed present, dormant, or genuinely missing before writing code, rather than assumed.

- **`event_milestones`** — Task gained `is_client_visible` (boolean, staff opts in PER TASK explicitly, never inferred from category — category-based inference was explicitly rejected as fragile). Dispatch lives in `Task::booted()`'s existing `updated()` hook (ONE centralized place), not duplicated across `TaskCenter::markDone()`/`updateStatus()`/`CreateTask::save()` separately.
- **`vendor_suggestions`** — already fully built by the Vendor Selection work above; this pass only renamed the category string from the generic `'vendors'` to the client-facing-distinct `'vendor_suggestions'`.
- **`vendor_bookings`** — hooked into `VendorDetail::saveEditAssignment()`, comparing old vs. new status, gated on `is_client_visible`.
- **`payments`** — hooked into `EventBudget::addPayment()`. Explicitly does NOT cover `InvoiceDetail::recordPayment()` (staff paying a vendor — internal accounting, never client-facing).
- **`rsvp`** — required NEW processing logic in `ProcessReminders.php` (a `processRsvpRule()` branch), since the existing reminder engine's own code comment explicitly listed `rsvp` as an acknowledged, never-built category. This is the ONE branch in that file that notifies a CLIENT rather than staff — every other branch notifies staff.
- **`event_details`** — hooked into `CreateEvent::save()`, diffing ONLY `date`/`venue`/`location` (not every field — an internal-notes edit never notifies the client).
- **Single funnel: `ClientNotificationService`.** Every hook calls one method on this service; the service alone checks `Tenant::clientNotificationEnabled($category)` (the tenant-wide master toggle) before ever calling `NotificationDispatchService::notify()`. No hook duplicates this check.
- **Tenant-wide master toggle is layered ON TOP OF the per-item flags, not a replacement for them.** For `event_milestones`/`vendor_bookings`, BOTH gates must pass (staff flagged the item AND the tenant has the category enabled). For the three automatic categories (`payments`/`rsvp`/`event_details`), the tenant toggle is the ONLY control, since there's no per-item flag to gate them.
- **All 6 categories default to ON** — matches the existing staff-notification-category default, and gives the feature value from day one without requiring tenant discovery/setup.
- **`client_notification_settings` stored as a single JSON column on `Tenant`** (mirroring the existing `branding` JSON pattern) rather than 6 separate boolean columns.

## ARCHITECTURE: VENDOR NOTIFICATIONS (4 categories)

Smaller than Client's set, deliberately — no tenant-wide master toggle was added (per-item/ownership gating was judged sufficient given the narrower scope).

- **`tasks`** — `SendTaskAssignedNotification` listener had an explicit code comment ("only staff (User) notifications for now — vendor notifications come in Stage 5") confirming this was a KNOWN, planned-but-unbuilt gap, not a dormant-but-reachable feature. Now branches to `VendorNotificationService::notifyTaskAssigned()` when `Task.vendor_account_id` is set.
- **`runsheets`** — closes a REAL, asymmetric gap: staff marking a runsheet item delayed already notified the vendor (`RunsheetItemDelayed` event, pre-existing), but a VENDOR marking their own item delayed notified no one. `VendorNotificationService::notifyRunsheetDelayedByVendor()` closes the reverse direction.
- **`bookings`** — new assignment created (`VendorDetail::assignToEvent()`) and status changed (`VendorDetail::saveEditAssignment()`), both notify the vendor's own `VendorAccount` (resolved via `Vendor` → `VendorAccount.vendor_id`, a different lookup path than Task's direct `vendor_account_id`).
- **`contracts`** — hooked into `ContractDetail::sendContract()`.
- **Confirmed and preserved architectural fact: vendor runsheet access in this app is OWNERSHIP-gated (`vendor_id` match), not PERMISSION-gated** — the seeded `vendor` role only lists `vendor.runsheet.view`, with no `.manage`/edit permission, yet `VendorRunsheet::updateStatus()` already lets vendors edit today via ownership match alone. Quick Access Links' vendor menu deliberately mirrors this same ownership-based gating rather than inventing a permission that doesn't reflect real existing behavior.
- **Found and fixed while building this: `Vendor\NotificationPreferences.php` had the identical `notifiable_type` string-literal bug already found and fixed on the Client side** (`'vendor_account'` instead of the real class `App\Models\Central\VendorAccount`) — meaning the vendor conversations-email toggle had never actually gated anything, silently always defaulting to "on" regardless of the switch shown.
- **Found and fixed while building this: `RsvpFormPage.php` had a hardcoded-role-NAME lookup** (`whereHas('roles', fn($q) => $q->where('name', 'company_owner'))`) for resolving who gets the planner RSVP-notification email — silently breaks if a tenant renames their owner role via Roles & Permissions. Fixed to use `is_system`, matching every other owner-lookup already fixed elsewhere in this app.

## ARCHITECTURE: RSVP RESPONSE-PERCENTAGE MILESTONES

- **Event-driven, not scheduled** — checked at the moment a guest's RSVP is recorded (`RsvpFormPage::submit()`), not polled on an offset/interval like the deadline reminder. Deliberate: there's no natural "offset" for a percentage threshold, and event-driven avoids the reminder engine's 15-minute polling-window edge cases entirely.
- **Hardcoded thresholds `[50, 75, 100]`**, not tenant-configurable — deliberately kept small since this was an explicitly-deferred, smaller add-on; a configuration UI for 3 numbers was judged disproportionate scope for this pass.
- **`RsvpForm.milestones_notified` (new nullable JSON column)** tracks which thresholds have already fired, preventing re-notification. Written via `forceFill()->save()` rather than a normal mass-assignment `update()`, since the actual `RsvpForm` model's fillable list was never confirmed in this session — `forceFill()` sidesteps the risk of a silent no-op if the column isn't listed there.
- **Reuses the existing `rsvp` category and tenant-wide toggle** — zero new settings required; this is a second KIND of RSVP notification riding the same on/off switch as the deadline reminder.

## ARCHITECTURE: QUICK ACCESS LINKS

The largest single feature of the session — permanent, personal, no-login links for staff AND vendors, explicitly treated as password-equivalent throughout.

- **Token: `Str::random(24)`**, generated via Laravel's `random_bytes()`-backed implementation. ~142 bits of entropy — deliberately well above the ~128-bit "unbreakable by brute force" threshold, chosen with margin specifically because this is a PERMANENT, reusable credential (unlike the one-off `DocumentShareLink` token elsewhere in this app), while remaining short enough to paste into SMS/WhatsApp.
- **One new polymorphic table: `quick_access_links`** (`person_type`/`person_id` — either `Tenant\User` or `Central\VendorAccount`), NOT per-model columns — gives a single place to list/audit every link across both person-types at once.
- **Regeneration = delete + recreate, never an in-place token update.** Guarantees the old link is PROVABLY dead the instant regeneration happens (an UPDATE could theoretically be read via stale cache/replica lag; a DELETE cannot), with zero overlap window where both old and new tokens work.
- **PIN is per-person, self-set, never tenant-wide** — explicitly discussed and confirmed: a shared team PIN barely improves security over no PIN, since it doesn't contain the blast radius to one compromised person. The TENANT controls the per-person REQUIREMENT (`pin_enabled`); the PERSON controls the PIN VALUE (`pin_hash`, bcrypt).
- **Every update action reuses the real Eloquent model's own `update()` call** (`Task`, `RunsheetItem`) rather than raw queries — meaning `Task::booted()`'s client-milestone-notification hook, and the existing `RunsheetItemDelayed`/vendor-delay notification logic, all fire automatically through Quick Access with zero separate integration code. Quick Access is a new FRONT DOOR into existing actions, not a parallel system.
- **Audit trail reuses the existing `ActivityTimeline` model** (`subject_type/id`, `event_type`, `description`, `actor_type/id`) — no new audit table built, since this already existed and fit the need exactly.
- **Menu content differs by person type based on what's ACTUALLY update-able today**, not an idealized permission list — staff's menu is `PermissionService`-gated (`tasks.edit`, `runsheet.manage`); vendor's menu is ownership-gated (matching the pre-existing real vendor-runsheet behavior noted above, not a fabricated permission).
- **Event-scoping**: staff via `EventTeam`, vendor via `VendorEventAssignment` — both existing models, no new scoping mechanism. A "General / Company Tasks" pseudo-option is prepended to the staff event dropdown specifically for tasks with `event_id = null`.
- **The public no-login UI is a SINGLE Alpine-driven page with zero reloads between screens** — menu → event picker → item list → status update, all client-side view-switching, with a "Back" button at every step. This directly answers "how does someone do a second action without needing the link again" — they simply navigate back within the same page visit; the link itself is meant to be bookmarked for future visits, not re-fetched per action.
- **Status updates are OPTIMISTIC** — the UI updates the instant a status is tapped, before the server confirms; only reverts if the server actually rejects it. Explicitly modeled on the same pattern already used for the mini-chat's local message echo elsewhere in this app.
- **Feature-gated via the EXISTING `FeatureGateService`/`FeatureFlag` system** (`quick_access_links` key), NOT a hardcoded tier check — the platform owner enables it per-plan through the pre-existing Plans UI (`CreatePlan.php`), exactly like every other feature flag in this app. No tier assumptions were baked into any code.
- **Three management surfaces, all built:** tenant admin panel (`QuickAccessAdmin` — list every staff+vendor, per-person PIN-requirement toggle, regenerate/deactivate/reactivate, per-person audit log viewer, Alpine-instant bulk-select for bulk-generate/bulk-deactivate), staff self-service (`Staff\QuickAccessSettings`, plus a matching topbar dropdown widget), vendor self-service (folded into the existing `Vendor\Profile` page, plus its own topbar widget).
- **Topbar widget redesign, mid-session:** the educational "what is this icon for" tip was INITIALLY nested inside the click-to-open dropdown — a real design mistake, since a tip meant to explain an icon BEFORE someone clicks it is useless hidden behind that same click. Corrected to a separate floating tip bubble that appears automatically on page load (independent Alpine state, own `localStorage` dismiss key), with the click-to-open dropdown remaining a completely separate, second floating element.
- **New reusable component: `<x-ui.dismissible-tip>`** — Hostinger-style contextual hint, `localStorage`-backed one-time dismiss, generic enough for reuse anywhere else in the app going forward.
- **New reusable component: `<x-ui.confirm-button>`** — replaces every remaining `wire:confirm` (native browser confirm dialog) in this feature with a custom Alpine-instant modal, matching the app's own visual design instead of an unstyled OS dialog. Established mid-session as the standard going forward for any future destructive-action confirmation.

## NEW FEATURES: CLIENTS & SETTINGS PAGES, SELF-SERVICE PROFILES

- **Two long-dead sidebar placeholders closed:** "Clients" and "Settings" had sat as literal `href="#"` links with no backing page since before this session — confirmed via direct sidebar file inspection, not assumed.
- **New `Tenant\Clients\ClientsList`** — lists every event with a client name/email set, cross-referenced against real `Client`/`ClientEventAccess` records, shows Invited/Not-Invited status, an Invite button (delegating to a newly-extracted `ClientInviteService`, reused identically from `EventDetail::inviteClient()` rather than duplicated), and a per-row revoke-access action. Explicitly scoped as REVOKING ACCESS TO ONE EVENT (deleting the `ClientEventAccess` row), NOT deleting the client's entire login account — since a client can have access to multiple events, a full-account delete triggered from one event's row would be unexpectedly destructive.
- **New `ClientInviteService`** — the invite-or-grant-existing-access logic extracted out of `EventDetail.php` into a shared service specifically so the new Clients page could reuse it without duplicating the branching logic (existing client with new event vs. brand-new client vs. already-granted).
- **New `Tenant\SettingsHub`** — a simple index/card-grid page linking out to every existing settings-adjacent page (Domain Settings, Clients, Client Notifications, Roles & Permissions, Quick Access Links, Billing, My Profile, My Quick Access Link), each card conditionally shown based on the SAME permission that already gates that destination page. Deliberately NOT a new settings subsystem — just a discoverability hub for pages that already existed scattered across the sidebar.
- **Self-service profile + password-change pages built for all three portals** (`Staff\Profile`, extended `Vendor\Profile`, new `Client\Profile`) — see Rule 103 above for full detail. Email is read-only in all three; password change requires current-password re-entry plus the existing uppercase/lowercase/digit complexity rule already used in `Onboarding.php`.
- **New permission: `clients.manage`** — gates the new Clients page, added to `company_owner` (always) and `coordinator` (per the same reasoning already applied to `vendors.client_involvement.manage`/`client-notifications.manage`).
- **New permission: `quick-access.manage`** — gates the tenant admin panel for Quick Access Links, same two roles.

---

## NEW COMMANDS / SETUP STEPS FROM THIS SESSION

```
php artisan koordli:resync-staff-role-templates   ← run after EVERY permission-seeder change this
                                                     session added (vendors.client_involvement.manage
                                                     was already covered; also now covers
                                                     client-notifications.manage, quick-access.manage,
                                                     clients.manage)
php artisan reverb:start                          ← must be running locally alongside `serve` and
                                                     `queue:work` for ANY real-time feature to work
                                                     (notification bells, conversation messages,
                                                     runsheet-delay broadcasts) — a common Windows
                                                     zombie-process gotcha documented in Rule 100
```

Notification templates seeded via Tinker this session (no seeder file exists for these — created directly, matching the established pattern from earlier sessions):
`document_shared_via_link`, `vendor_suggestion_received`, `vendor_suggestion_decided`, `client_suggested_vendor`, `client_vendor_decision`, `client_task_milestone_completed`, `client_vendor_booking_status_changed`, `client_payment_recorded`, `client_event_details_changed`, `client_rsvp_deadline_approaching`, `client_rsvp_milestone_reached`, `vendor_task_assigned`, `vendor_marked_delayed`, `vendor_booking_created`, `vendor_booking_status_changed`, `vendor_contract_sent`, `quick_access_link_regenerated`.
...
```
Migrations: client_vendor_involvement_level+vendor_disclaimer_text on tenants,
  selection_source+client_approval_status+is_client_visible+client_can_view_pricing+
  payment_responsibility+disclaimer_acknowledged_at on vendor_event_assignments,
  source+added_by_client_id on vendors, create_vendor_suggestions_table,
  client_vendor_involvement_override on events, is_client_visible on tasks,
  client_notification_settings on tenants, milestones_notified on rsvp_forms,
  create_quick_access_links_table

app/Models/Tenant/VendorSuggestion.php                      ← new
app/Services/ClientNotificationService.php                  ← new, single funnel for all 6 client categories
app/Services/Notifications/VendorNotificationService.php    ← new, single funnel for all 4 vendor categories
app/Services/ClientInviteService.php                        ← new, extracted from EventDetail
app/Services/QuickAccessService.php                         ← new, menu/event/item resolution logic
app/Models/Tenant/QuickAccessLink.php                        ← new
app/Http/Controllers/QuickAccessController.php               ← new, public update-task/update-runsheet endpoints
app/Livewire/Public/QuickAccessPage.php                       ← new, the no-login menu/action UI
app/Livewire/Tenant/QuickAccessAdmin.php                      ← new, tenant management panel
app/Livewire/Tenant/QuickAccessWidget.php                     ← new, topbar dropdown
app/Livewire/Vendor/QuickAccessWidget.php                     ← new, topbar dropdown
app/Livewire/Staff/QuickAccessSettings.php                    ← new, self-service page
app/Livewire/Staff/Profile.php                                ← new
app/Livewire/Client/Profile.php                               ← new
app/Livewire/Tenant/Vendors/VendorSuggestions.php             ← new
app/Livewire/Client/Vendors/VendorList.php                    ← new
app/Livewire/Tenant/Clients/ClientsList.php                   ← new
app/Livewire/Tenant/SettingsHub.php                           ← new
resources/views/components/ui/dismissible-tip.blade.php       ← new, reusable
resources/views/components/ui/confirm-button.blade.php        ← new, reusable
app/Console/Commands/ProcessReminders.php                     ← new processRsvpRule() branch added
app/Listeners/SendTaskAssignedNotification.php                ← now branches to vendor path
app/Livewire/Public/RsvpFormPage.php                          ← is_system fix (was hardcoded role name)
app/Livewire/Vendor/NotificationPreferences.php                ← notifiable_type bug fixed
app/Livewire/Vendor/VendorRunsheet.php                         ← delay-notification hook added
app/Livewire/Tenant/Vendors/VendorDetail.php                   ← booking-created/changed hooks (both client + vendor)
app/Livewire/Tenant/Budget/EventBudget.php                     ← payment-recorded hook
app/Livewire/Tenant/Events/CreateEvent.php                     ← event-details-changed hook + per-event override field
app/Livewire/Tenant/Contracts/ContractDetail.php               ← contract-sent-to-vendor hook
app/Models/Tenant/Task.php                                      ← updated() hook now dispatches client milestone notification
app/Models/Tenant/Event.php                                     ← effectiveClientVendorInvolvementLevel()
app/Models/Central/Tenant.php                                   ← clientNotificationEnabled()
```
...
- **No cross-tenant shared identity for vendors/clients** — explicitly discussed and deliberately left as-is (see Rule 104). A vendor/client working with multiple tenants gets a fully separate account per tenant today.
- **`quick_access_links` has no scheduled cleanup for genuinely abandoned/never-used links** — unlike the chunked-upload-session and PWA-icon cleanup commands built earlier in this project, no equivalent exists for stale Quick Access links (e.g. a deactivated staff member's link sitting inert forever). Low priority, no user-facing symptom, but noted for parity with this app's established cleanup-command pattern.
- **RSVP milestone thresholds are hardcoded, not tenant-configurable** — deliberate scope limit for this pass (see Rule under RSVP Milestones architecture above); a future enhancement, not a bug.
- **No vendor-side equivalent of the tenant-wide "Client Notifications" master toggle** — Vendor's 4 categories rely solely on the existing per-category email-preference page plus ownership/assignment gating; if a tenant wants to blanket-disable a whole vendor-notification category the way they can for clients, that UI doesn't exist yet.