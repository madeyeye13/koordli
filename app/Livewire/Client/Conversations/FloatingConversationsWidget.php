...
class FloatingConversationsWidget extends Component
{
    private function buildList(): array
    {
        ...
        return $conversations->map(function ($conv) use ($client) {
            $myRow = ConversationParticipant::where('conversation_id', $conv->id)
                ->where('participant_type', 'client')->where('participant_id', $client->id)->first();
            ...
            $unread = ConversationMessage::where('conversation_id', $conv->id)
                ...
                ->where(fn($q) => $q->where('sender_type', '!=', 'client')->orWhere('sender_id', '!=', $client->id))
                ->count();
    ...
    public function render()
    {
        ...
        return view('livewire.client.conversations.floating-conversations-widget', [
